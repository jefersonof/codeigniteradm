...
<header>
	...
	<div class="heroe">
		...
		<?php
		// vê se tem msg de sucesso ou erro
		echo '<div style="background: lightgreen; padding: 16px 20px">';
		echo $session->getFlashdata('sucesso');
		echo '</div>';

		// msgs de validação de erros
		$validation =  \Config\Services::validation();
		echo $validation->listErrors();
		?>

		<h1>Exemplo bem básico de Cadastro</h1>
		...
		<input type="password" name="pswConf" placeholder="Confirmar Senha" required />
		<input type="tel" name="celular" placeholder="Celular" value="<?= set_value('celular') ?>" required />
...
<section>
	...
	<h1>Pessoas Cadastradas (clique para editar)</h1>

	<?php foreach ( $pessoas as $p ) {
		echo '<pre>
				<code>
					<a href="'.base_url('home/editar/'.$p->id).'">'.
...
<div class="further">
	...
	<section>
		...
		<h1>Clique numa pessoa pra deletar</h1>

		<?php foreach ( $pessoas as $p ) {
		echo '<pre>
				<code>
					<a href="'.base_url('home/deletar/'.$p->id).'">'.
						$p->nome.' - '.$p->email.' - '.$p->celular.
					'</a>
				</code>
			  </pre>';
